...
export const buildCharmPrompt = (topic: string, tone = ""): string => {
  ...
  prompt["Title"] = "Translate '" + topic + "' to native Portuguese"
  prompt["Subtitle"] = `Fill the blank in native Portuguese based on the context`
  prompt["Message Prompt"] = "Refer the title to write a phrase in native Portuguese"
  prompt["Tone"] = fallback(tone, "Choose from Romance, Atitude, or Sedução")

  for (let i = 1; i <= 5; i++) {
    prompt[`Don't Say Message ${i}`] = `Fill the blank in native Portuguese based on the context`
    prompt[`Say Message ${i}`] = `Fill the blank in native Portuguese based on the context`
  }

  return `You are writing a TikTok post teaching women how to text with men and providing "don't say" messages and "say" messages. Here is the structure of your post. If content is provided, you must not change the content in that field unless the field requires to use Portuguese. If you need to fill in blanks, fill them based on the overall context of the post. Here are some examples of extremely viral post, but they are all in English. You will need to use Portuguese in some fields as required. Learn from them and write a viral post. In terms of the messages generated, they must not use any emoji. For "say" messages, they should be VERY impressive.
Example 1:
{
...
/* ------------------------ API CALLER ------------------------ */
export async function generatePTCarousel({
  topic,
  tone = "",
  textStyle = "style1",
