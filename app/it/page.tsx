...
import ITUploadScreen from '@/components/it/ItUploadScreen'
import { generateITCarousel } from '@/lib/generate-it'
import { useITStore } from '@/lib/store/itStore'
import ITPreviewScreen from '@/components/it/ItPreviewScreen'
...
export default function Page() {
    ...
    const setData = useITStore((s) => s.setData)
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const { data: response, error } = await generateITCarousel({ topic: topicTitle })
    ...
    return (
        <main className="flex flex-col h-full">
            {currentScreen === 'upload' ? (
                <ITUploadScreen onGenerate={handleGenerate} loading={loading} />
            ) : (
                <ITPreviewScreen images={previewImages} />
            )}
        </main>
    )
}
