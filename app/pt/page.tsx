...
import PTUploadScreen from '@/components/pt/PtUploadScreen'
import { generatePTCarousel } from '@/lib/generate-pt'
import { usePTStore } from '@/lib/store/ptStore'
import PTPreviewScreen from '@/components/pt/PtPreviewScreen'
...
export default function Page() {
    ...
    const setData = usePTStore((s) => s.setData)
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const { data: response, error } = await generatePTCarousel({ topic: topicTitle })
    ...
    return (
        <main className="flex flex-col h-full">
            {currentScreen === 'upload' ? (
                <PTUploadScreen onGenerate={handleGenerate} loading={loading} />
            ) : (
                <PTPreviewScreen images={previewImages} />
            )}
        </main>
    )
}
