...
import FRUploadScreen from '@/components/fr/FrUploadScreen'
import { generateFRCarousel } from '@/lib/generate-fr'
import { useFRStore } from '@/lib/store/frStore'
import FRPreviewScreen from '@/components/fr/FrPreviewScreen'
...
export default function Page() {
    ...
    const setData = useFRStore((s) => s.setData)
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const { data: response, error } = await generateFRCarousel({ topic: topicTitle })
    ...
    return (
        <main className="flex flex-col h-full">
            {currentScreen === 'upload' ? (
                <FRUploadScreen onGenerate={handleGenerate} loading={loading} />
            ) : (
                <FRPreviewScreen images={previewImages} />
            )}
        </main>
    )
}
