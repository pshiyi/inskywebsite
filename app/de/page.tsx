...
import DEUploadScreen from '@/components/de/DeUploadScreen'
import { generateDECarousel } from '@/lib/generate-de'
import { useDEStore } from '@/lib/store/deStore'
import DEPreviewScreen from '@/components/de/DePreviewScreen'
...
export default function Page() {
    ...
    const setData = useDEStore((s) => s.setData)
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const { data: response, error } = await generateDECarousel({ topic: topicTitle })
    ...
    return (
        <main className="flex flex-col h-full">
            {currentScreen === 'upload' ? (
                <DEUploadScreen onGenerate={handleGenerate} loading={loading} />
            ) : (
                <DEPreviewScreen images={previewImages} />
            )}
        </main>
    )
}
