...
import { useITStore } from '@/lib/store/itStore'
...
interface itPreviewScreenProps {
  images: string[]
}
...
/* ---------------------------- 🔥 MAIN COMPONENT --------------------------- */
export default function ITPreviewScreen({ images }: itPreviewScreenProps) {
  const data = useITStore(state => state.data);
  ...
  const finalResult = 'Messaggi suggeriti da CharmChat\n' + messages
    .map(msg => `Non dire: "${msg.text}"\nDì invece: "${msg.description}"`)
    .join('\n') + '\n#dating #regina #relazione #mensagens '
        ...
        {images.map((img, index) => {
          ...
          return (
            <MessagePage
              key={`msg-${index}`}
              image={img}
              message={msg?.text ? `❌ Non dire: "${msg.text}"` : ''}
              description={msg?.description ? `✅ Dì invece: "${msg.description}"` : ''}
              downloadIndex={sequentialIndex}
            />
          )
        })}
      ...
      className="relative cursor-pointer" style={{
        ...
        <div className={`w-[308px] scale-[0.60] p-2 px-4 flex flex-col gap-4 absolute left-0 z-30 bg-[#FAFAFA] text-gray-900 shadow-lg ... border border-gray-200 ${poppins.className}`} style={{ top: `${topOffset}px` }}>
          <div className="absolute scale-[1.05] flex left-[200px] -top-36 flex-col z-40 items-end space-y-2 text-right text-[11px] text-white">
            ...
            Copia e incolla per farlo impazzire di te
          ...
          <div className="flex justify-between font-medium items-center px-4 text-sm text-gray-600">
            ...
            <span className="text-[12px] text-[#8063EF] font-medium flex gap-2 items-center"><img src={'/charmchat/Adjust.svg'} className='w-3 h-3' />Modifica</span>
          </div>
